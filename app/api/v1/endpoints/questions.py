...
async def submit_answer(
    ...
    try:
        ...
        if not user_answer or len(user_answer.strip()) < 3:
            raise ValueError("Transcription too short or unclear")
    ...
    if existing and existing.get("user_answer"):
        return JSONResponse(status_code=409, content={"error": f"Answer already submitted for Q{question_id}."})
...
async def mark_for_review(candidate_id: str = Form(...), question_id: int = Form(...)):
    ...
    existing = get_question_entry(session, question_id)
    if not existing or not existing.get("user_answer"):
...
async def get_review_questions(candidate_id: str):
    ...
    review_questions = [q for q in session["questions"] if q["marked_for_review"]]
