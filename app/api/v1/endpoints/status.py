...
async def detect_head_pose(
    ...
    try:
        ...
        # Mild pre-processing to stabilize detection
        # 1) Resize to a reasonable width to reduce noise/blur variability
        h, w = img.shape[:2]
        target_w = 640
        ...
        if rvec is None:
            # Quick retry with mild enhancement (helps in low-light/blur)
            time.sleep(0.08)
            enhanced = cv2.convertScaleAbs(img, alpha=1.2, beta=10)
            rvec, tvec, _ = pose_estimator.estimate_pose(enhanced)
        ...
        return {
            ...
            "cheating": cheating,
            "violation": cheating,
            "status": "violation" if cheating else "ok",
            "message": reason if cheating else None,
            "reason": reason
        }
